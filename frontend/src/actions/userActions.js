...
export const register = (first_name, email, password) => async (dispatch) => {
    try {
        ...
        const config = {headers: {'Content-type': 'application/json'}}
        const { data } = await axios.post('/api/users/register/',{ 'first_name': first_name, 'email': email, 'password': password },config)
...
export const updateUserProfile = (id,first_name,email,password) => async (dispatch, getState) => {
    try {
        dispatch({type: USER_UPDATE_PROFILE_REQUEST})
        const {userLogin: { userInfo },} = getState()
        const config = {headers: {'Content-type': 'application/json',Authorization: `Bearer ${userInfo.token}`}}
        const { data } = await axios.put(`/api/users/profile/update/`,{'_id':id,'first_name': first_name, 'email': email, 'password': password},config)
